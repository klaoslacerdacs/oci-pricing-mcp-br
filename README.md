...
- **Live multicloud compute.** `get_cloud_instance_price` and `compare_vm_oci_vs_cloud` pull **live** AWS/Azure/GCP instance pricing by proxying the public [Vantage instances MCP](https://instances.vantage.sh) (per-instance, avoids the unusable ~316 MB raw dumps). `compare_vm_oci_vs_cloud` applies the **OCPU↔vCPU de-para** (1 OCPU = 2 vCPU, RAM 1:1) against the OCI E5 shape. Endpoint overridable via env `VANTAGE_MCP_URL`. This replaces the old hardcoded competitor constants for compute — egress/k8s comparisons remain approximate. **Burstable de-para (T2/T3/T3a/T4g):** AWS T-family instances are matched to OCI **burstable VMs** by sustained capacity — the AWS per-vCPU baseline (e.g. t3.medium 20% × 2 vCPU = 0.4 sustained vCPU) picks the smallest OCI baseline (12.5% or 50%) that covers it, and OCI compute is billed at that baseline fraction (e.g. t3.medium $30.37 vs OCI 1 OCPU @ 50% = **$16.79/mo**). Caveat noted in output: AWS credits/surplus billing vs OCI free-but-not-guaranteed bursting.
...
- **Service minimums enforced.** Autonomous DB floor is **2 ECPUs** (`calculate_database_cost` clamps and notes it; 2-ECPU ATP + 20 GB ≈ **$506/mo** at 744h — ATP DB storage corrected to `B95706` $0.1953/GB/mo, backup `B95754` flagged as billed separately). MySQL floor is **2 ECPUs + 50 GB** (`B108030` $0.0366/ECPU-hr + `B92426` $0.04/GB/mo ≈ **$56.46/mo** at 744h; prices corrected from stale OCPU model). OpenSearch has **no standalone node**: minimum deployment is a full cluster (leader + data + dashboard E4 nodes `B93113`/`B93114` + HA node `B93709` $0.25/hr + block volume) — practical floor **~$313/mo**.
- **Validated against Oracle's own Cloud Estimator data.** All SKU prices cross-checked against `oracle.com/a/ocom/docs/cloudestimator2/data/products.json` (654 SKUs, PAYG in every currency) and shape minimums against `.../shapes.json`: 602 of 604 dual-currency SKUs share the exact BRL/USD ratio **5.2357** (the 2 outliers are support services priced 1:1). Fixed from that sweep: A1/Standard3/Optimized3 memory $0.0015 (was inflated), DenseIO.E4.Flex OCPU $0.025 + NVMe B93123 billed separately, flex max limits (E6 126 OCPU, A1 76/472). `calculate_monthly_cost` now clamps to shape hardware floors (DenseIO flex min 8 OCPU/128 GB). Note on BRL: Oracle's list BRL is FX-only (B95702 = R$1.7592 ≈ USD×5.2357, **no tax**); `convert_usd_brl` additionally grosses up Brazilian invoice taxes (÷0.87 → R$2.02) — use the tool for what you'll actually pay. A1's $0 list price is a free tier band (first 3000 OCPU-hrs/mo), then $0.01 — estimates use the paid band.
