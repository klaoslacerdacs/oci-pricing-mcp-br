...
- **Validated against Oracle's own Cloud Estimator data.** All SKU prices cross-checked against `oracle.com/a/ocom/docs/cloudestimator2/data/products.json` (654 SKUs, PAYG in every currency) and shape minimums against `.../shapes.json`. Note on BRL: Oracle's list BRL price is FX-only (e.g. B95702 = R$1.7592/ECPU-hr ≈ USD×5.236); `convert_usd_brl` additionally grosses up Brazilian invoice taxes (÷0.87 → R$2.02) — use the tool for what you'll actually pay, the list price for what Oracle publishes.
